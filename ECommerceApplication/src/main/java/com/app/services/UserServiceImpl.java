...
public class UserServiceImpl implements UserService {
	...
	public UserDTO registerUser(UserDTO userDTO) {
		...
		try {
			// userDTO now comes in with a PLAIN TEXT password
			User user = modelMapper.map(userDTO, User.class);

			// ADD THIS LINE TO ENCODE THE PASSWORD
			...
			// Use the saved user to create the final DTO
			UserDTO savedUserDTO = modelMapper.map(registeredUser, UserDTO.class);
			savedUserDTO.setAddress(modelMapper.map(registeredUser.getAddresses().stream().findFirst().get(), AddressDTO.class));
